...
###here some stuff that you wrote before
...
betKatz_sorted = dict(sorted(nx.katz_centrality(G).items(), key=lambda item: item[1], reverse=True))
print(betCent_sorted)
print(betClos_sorted)
print(betEige_sorted)
print(betKatz_sorted)
...
colors = []
for n in G.nodes:
    colors.append(float(nx.katz_centrality(G)[n]))
nx.spring_layout(G)
nx.draw(G, with_labels=True, node_color=colors, cmap=plt.colormaps['Blues'])
plt.show()

# Wow! 0 appears to be always the most central person. But is it always the case?
...
colors = []
for n in G1.nodes:
    colors.append(float(nx.katz_centrality(G1)[n]))
nx.spring_layout(G1)
nx.draw(G1, with_labels=True, node_color=colors, cmap=plt.colormaps['Blues'], pos=pos)
plt.show()
...
You can pick whatever network you want but consider that networks with more than 500 nodes start to be heavy. Something 
with 30/200 nodes should be good!
...
Try to represent graphically the network you find and to write a quick report with possible explanation regarding what 
are the possible reasons for discrepancies in terms of centrality in your specific network (consider the subject and the
type of network you're observing). 

Include everything in a .py file and send it to sasha.piccione@unive.it

Remember that the .py script must be executable.
